      ...
      <div className="double-bezel-core flex-1 flex flex-col overflow-hidden">
        ...
        {isRecording && (
          <motion.div
            ...
            className="px-5 py-2.5 bg-rose-600 text-white text-xs font-mono font-bold flex items-center justify-between shadow-md"
          >
            <div className="flex items-center gap-2">
              <span className="w-2.5 h-2.5 rounded-full bg-white animate-ping" />
              <span>LIVE MIC ACTIVE — Speak any emergency aloud in English (e.g. "my roommate collapsed, not breathing")...</span>
            </div>
            <button
              onClick={toggleMic}
              className="text-white bg-white/20 hover:bg-white/30 px-2.5 py-1 rounded-full text-[10px] uppercase font-mono cursor-pointer transition-colors"
            >
              Stop Mic
            </button>
          </motion.div>
        ...
        <div className="p-4 border-b border-slate-100 bg-slate-50/40">
          <div className="flex items-center justify-between mb-3">
            <span className="text-[10px] font-mono uppercase tracking-wider text-slate-600 font-bold flex items-center gap-1.5">
              ...
              Reviewer Presets (Quick-Test Benchmarks)
            </span>
        ...
        {/* Quick Freeform Prompt Suggestions */}
        <div className="px-4 py-2 bg-slate-50/90 border-t border-slate-200/70 flex items-center gap-1.5 overflow-x-auto text-[10px] font-mono text-slate-500">
          ...
          {[
            ...
            'Severe bee sting tongue swelling',
            'Grandmother left side face drooping',
            'Toddler swallowed bleach foaming mouth',
            'Sudden chest pain sweating left arm numbness',
          ].map((promptText) => (
            <button
              key={promptText}
              type="button"
              onClick={() => {
                setCustomInput('');
                setActivePresetId(null);
                onProcessTranscript(promptText);
              }}
              className="px-2.5 py-1 rounded-full bg-white hover:bg-rose-50 hover:border-rose-300 hover:text-rose-700 border border-slate-200 text-slate-700 font-medium whitespace-nowrap transition-colors cursor-pointer shadow-2xs"
            >
              "{promptText}"
            </button>
          ))}
